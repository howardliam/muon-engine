...
namespace muon::engine {
    ...
    /* Device */
    Device::Device(logging::Logger logger, window::Window &window, toml::table &config) : logger(logger), window(window) {
        enableValidationLayers = config["vulkan"]["validation_layers"].value<bool>().value_or(false);
    ...
    Device::~Device() {
        allocator.destroy();
        logger->debug("Destroyed allocator");

        device.destroy(nullptr);
        logger->debug("Destroyed device");

        instance.destroySurfaceKHR(surface, nullptr);
        logger->debug("Destroyed surface");

        destroyDebugUtilsMessenger(instance, debugMessenger, nullptr);
        logger->debug("Destroyed debug messenger");
    ...
    void Device::createDebugMessenger() {
        ...
        logger->debug("Created debug messenger");
    }
    ...
    void Device::selectPhysicalDevice() {
        ...
        logger->debug("Selected a physical device: {}", deviceProperties.deviceName.data());
    }
    ...
    SwapchainSupportDetails Device::querySwapchainSupport(vk::PhysicalDevice physicalDevice) {
        ...
        if (result != vk::Result::eSuccess) {
            logger->warn("Failed to get surface capabilities");
        }

        uint32_t formatCount;
        result = physicalDevice.getSurfaceFormatsKHR(surface, &formatCount, nullptr);
        if (result != vk::Result::eSuccess) {
            logger->warn("Failed to get surface formats");
        }
        ...
        if (result != vk::Result::eSuccess) {
            logger->warn("Failed to get surface present modes");
        }
