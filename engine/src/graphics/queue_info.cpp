...
namespace muon::gfx {
    ...
    bool QueueFamilyInfo::IsPresentCapable() const {
        return capabilities.test(3);
    }

    bool QueueFamilyInfo::IsGraphicsCapable() const {
        return capabilities.test(2);
    }

    bool QueueFamilyInfo::IsComputeCapable() const {
        return capabilities.test(1);
    }

    bool QueueFamilyInfo::IsTransferCapable() const {
        return capabilities.test(0);
    }

    bool QueueFamilyInfo::IsComputeDedicated() const {
        return !capabilities.test(2) && capabilities.test(1);
    }
    ...
    QueueInfo::QueueInfo(VkPhysicalDevice physicalDevice, VkSurfaceKHR surface) {
        ...
        for (uint32_t i = 0; i < queueFamilyPropertyCount; i++) {
            ...
            if (presentSupport) { capabilities.set(3); }
            if (properties.queueFlags & VK_QUEUE_GRAPHICS_BIT) { capabilities.set(2); }
            if (properties.queueFlags & VK_QUEUE_COMPUTE_BIT) { capabilities.set(1); }
