#include "muon/graphics/pipeline_mesh.hpp"
...
namespace muon::graphics {
    ...
    PipelineMeshState::PipelineMeshState() {
    ...
    PipelineMesh::PipelineMesh(const PipelineMeshSpecification &spec) : m_device(spec.device), m_layout(spec.layout), m_state(spec.state) {
        CreateCache();
        CreateShaderModules(spec.paths);
    }

    PipelineMesh::~PipelineMesh() {
    ...
    void PipelineMesh::Bake(const VkPipelineRenderingCreateInfo &renderingCreateInfo) {
        if (m_pipeline) {
            vkDestroyPipeline(m_device, m_pipeline, nullptr);
        }
    ...
    void PipelineMesh::Bind(VkCommandBuffer cmd, const std::vector<VkDescriptorSet> &sets) {
    ...
    void PipelineMesh::CreateShaderModules(const PipelineMeshShaderPaths &paths) {
        std::map<VkShaderStageFlagBits, std::filesystem::path> shaderPaths;

        if (paths.taskPath) {
            shaderPaths[VK_SHADER_STAGE_TASK_BIT_EXT] = *paths.taskPath;
        }
        shaderPaths[VK_SHADER_STAGE_MESH_BIT_EXT] = paths.meshPath;
    ...
    void PipelineMesh::CreatePipeline(const VkPipelineRenderingCreateInfo &renderingCreateInfo) {
